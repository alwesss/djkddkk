...
public class SoyoAccessibilityService extends AccessibilityService {
    ...
    private void scrollRecommendationsForward(List<AccessibilityNodeInfo> nodes) {
        long now = SystemClock.elapsedRealtime();
        if (now - lastAutoScrollAt < 1400) return;

        AccessibilityNodeInfo best = null;
        int bestArea = -1;
        for (AccessibilityNodeInfo node : nodes) {
            if (node == null || !node.isVisibleToUser() || !node.isScrollable()) continue;
            if (!node.getActionList().contains(AccessibilityNodeInfo.AccessibilityAction.ACTION_SCROLL_FORWARD)) continue;
            Rect r = new Rect();
    ...
    private boolean looksLikeRecommendationsScreen(List<AccessibilityNodeInfo> nodes) {
        ...
        for (AccessibilityNodeInfo node : nodes) {
            ...
            if (isChatLabel(text) || isChatLabel(desc)) chatButtons++;
        }
        return hasRecommendations && chatButtons > 0;
    }
    ...
    private void prepareChatMessage(AccessibilityNodeInfo root, AccessibilityNodeInfo editor) {
        ...
        carriedRecommendationName = null;
        carriedRecommendationAt = 0L;
        if (isFullAutoEnabled() && send != null) {
    ...
    private void scheduleAutomaticSend(String expectedName) {
        ...
        mainHandler.postDelayed(() -> {
            ...
            if (!isFullAutoEnabled() || TextUtils.isEmpty(expectedName)
                    || !expectedName.equals(pendingName)) return;

            AccessibilityNodeInfo freshRoot = getRootInActiveWindow();
            AccessibilityNodeInfo freshSend = null;
            if (freshRoot != null) {
                AccessibilityNodeInfo freshEditor = findBestEditor(freshRoot);
                if (freshEditor != null) freshSend = findSendButton(freshRoot, freshEditor);
            ...
            if (!clicked) {
                showApproval(expectedName, pendingMessage == null ? "" : pendingMessage,
                        pendingSendButton != null);
                Toast.makeText(this, "Otomatik gönderilemedi; onay ekranı açıldı.", Toast.LENGTH_SHORT).show();
                return;
            }
    ...
    private void returnToRecommendations() {
        ...
        // Bazı SOYO sürümlerinde ok tıklaması ilk seferde işlenmeyebiliyor. Sohbet kutusu
        // hâlâ görünüyorsa bir kez Android geri işlemiyle tamamla; sonra liste taramasını başlat.
        mainHandler.postDelayed(() -> {
            ...
            if (currentRoot != null) {
                AccessibilityNodeInfo editor = findBestEditor(currentRoot);
                stillInChat = editor != null;
                recycleNode(editor);
                currentRoot.recycle();
            }
            if (stillInChat) performGlobalAction(GLOBAL_ACTION_BACK);
            returningToRecommendations = false;
            mainHandler.postDelayed(this::scanSoyoScreen, 900);
        }, 1600);
    }
